...
function launch(version, username, uuid = "a", javaP) {
	...
	const {
		mainClass
               ...
               const classpath = (minecraft.inheritsFrom ? resolve("./minecraft/versions/" + minecraft.inheritsFrom + "/" + minecraft.inheritsFrom + ".jar;") : "") + resolve(clientJar) + ";" + libs.map(e => resolve("./minecraft/libraries/" + (e?.downloads?.artifact?.path || e?.downloads?.classifiers["natives-" + (process.platform == "win32" ? "windows" : (process.platform == "darwin" ? "macos" : "linux"))]?.path || parseLib(e?.downloads?.artifact?.name || e?.downloads?.classifiers?.name || e?.name))))
		.join(";") + ";" + (minecraft.inheritsFrom ? (JSON.parse(fs.readFileSync("./minecraft/versions/" + minecraft.inheritsFrom + "/" + minecraft.inheritsFrom + ".json")).libraries.map(e => resolve("./minecraft/libraries/" + (e?.downloads?.artifact?.path || e?.downloads?.classifiers["natives-" + (process.platform == "win32" ? "windows" : (process.platform == "darwin" ? "macos" : "linux"))]?.path || parseLib(e?.downloads?.artifact?.name || e?.downloads?.classifiers?.name || e?.name))))).join(";") : "");
